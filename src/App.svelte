<script>
  ...
  async function deletePour(pour) {
    ...
    if (error) appError = error.message
    else pours = pours.filter((item) => item.id !== pour.id)
  ...
  async function deleteJob(job) {
    ...
    if (error) appError = error.message
    else jobs = jobs.filter((item) => item.id !== job.id)
  ...
  <main class="auth-page">
    <section class="auth-card">
      ...
      <form on:submit={signIn} class="auth-form">
        ...
        <button class="button primary wide" type="submit" disabled={signingIn}>
  ...
  <div class="app-shell">
    <header class="topbar">
      <div class="topbar-inner">
        <button class="brand-button" on:click={backToJobs} aria-label="Go to Jobs">
          <span class="brand-mark small">DT</span>
          <span>
            <span class="eyebrow">DataTracker</span>
        ...
        <div class="user-menu">
          {#if employee}
            <span class="user-name">{employeeName(employee)}</span>
          {/if}
    ...
    <main class="content">
      {#if appError}
        <div class="alert error top-alert">{appError}</div>
      {/if}
      {#if notice}
        <div class="alert success top-alert">{notice}</div>
      {/if}
      ...
      {:else if view === 'jobs'}
        <section class="page-heading">
          <div>
            ...
            <h1>Jobs</h1>
            <p>Track each job and its associated pour stages.</p>
          </div>
          <button class="button primary" on:click={createJob} disabled={creatingJob}>
        ...
        <section class="panel list-panel">
          ...
          {:else if jobs.length === 0}
            <div class="empty-state compact">
              <h3>No jobs yet</h3>
              <p>Create your first Job to start tracking Pours.</p>
              <button class="button primary" on:click={createJob}>New Job</button>
            </div>
          {:else}
              ...
              <table>
                ...
                <tbody>
                  {#each jobs as job}
                    <tr>
                      <td>
                        <button class="record-link" on:click={() => openJob(job.id)}>Job {job.seq}</button>
                      </td>
                      <td class="row-actions">
                        <button class="button secondary small-button" on:click={() => openJob(job.id)}>Open</button>
                        <button
                          class="icon-button danger"
                          title="Delete job"
                          aria-label={`Delete Job ${job.seq}`}
        ...
        {:else if selectedJob}
          <section class="detail-heading">
            <div>
              ...
              <h1>Job {selectedJob.seq}</h1>
              <p class="creator-line">
                Created {formatDateTime(selectedJob.created_at)}
                {#if selectedJob.creator} by {employeeName(selectedJob.creator)}{/if}
              </p>
            </div>
          </section>

          <section class="panel list-panel">
            <div class="panel-heading pours-heading">
              <div>
                <p class="eyebrow">Pours</p>
                <h2>Stages on this job</h2>
              </div>
              <div class="heading-actions">
                <span class="count-badge">{pours.length}</span>
                <button class="button primary" on:click={createPour} disabled={creatingPour}>
                  <span class="plus">+</span>
                  {creatingPour ? 'Adding…' : 'Add Pour'}
                </button>
              </div>
            </div>
            ...
            {#if pours.length === 0}
              <div class="empty-state compact">
                <h3>No Pours yet</h3>
                <p>The first Pour added to this Job will automatically receive Stage 1.</p>
                <button class="button primary" on:click={createPour}>Add Pour</button>
              </div>
            {:else}
              <div class="table-wrap">
                <table>
                  ...
                  <tbody>
                    {#each pours as pour}
                      <tr>
                        <td><strong>Stage {pour.stageNum}</strong></td>
                        <td class="muted-cell">{formatDateTime(pour.created_at)}</td>
                        <td class="muted-cell">{employeeName(pour.creator)}</td>
                        <td class="row-actions">
                          <button
                            class="icon-button danger"
                            title="Delete pour"
                            aria-label={`Delete Stage ${pour.stageNum}`}
                            disabled={deletingPourId === pour.id}
                            on:click={() => deletePour(pour)}
                          >
                            {deletingPourId === pour.id ? '…' : '×'}
                          </button>
                        </td>
